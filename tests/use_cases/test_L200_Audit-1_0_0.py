...
from .conftest import AssertFailureRolesMixin, replace_element, remove_element

BASE_DIR = Path(__file__).parent.parent.parent / 'ASHRAE_211'
L200_schematron = str(BASE_DIR / 'schematron' / 'L200_Audit-1.0.0.sch')
L200_example = str(BASE_DIR / 'examples' / 'L200_Audit-1.0.0.xml')
...
class TestL200AuditMiscellaneous(AssertFailureRolesMixin):
    schematron = L200_schematron
    exemplary_file = L200_example

    def test_is_invalid_when_schedule_types_differ_inside_schedule_details(self):
        # -- Setup
        tree = etree.parse(L200_example)
...
class TestL200AuditHvacSystems(AssertFailureRolesMixin):
    schematron = L200_schematron
    example_file = str(test_data_dir / 'HVAC_example1.xml')
...
class TestL200AuditDhwSystems(AssertFailureRolesMixin):
    schematron = L200_schematron
    example_file = str(test_data_dir / 'DHW_example1.xml')
...
class TestL200AuditLightingSystems(AssertFailureRolesMixin):
    schematron = L200_schematron
    example_file = str(test_data_dir / 'Lighting_example1.xml')
...
class TestL200AuditLoads(AssertFailureRolesMixin):
    schematron = L200_schematron
    example_file = str(test_data_dir / 'Loads_example1.xml')
...
class TestL200AuditSectionSystems(AssertFailureRolesMixin):
    schematron = L200_schematron
    example_file = str(test_data_dir / 'Section_System_Links.xml')
